...
func (server *Server) DeleteCommentRelation(ctx context.Context, req *rpcs.DeleteCommentRelationRequest) (*rpcs.DeleteCommentRelationResponse, error) {
	apiUserDailyLimit := 1000
	apiKey := "DeleteCommentRelation"
	authPayload, err := server.authUser(ctx, []string{util.AdminRole, util.UserRole}, apiUserDailyLimit, apiKey)
	if err != nil {
		return nil, err
	}
	violations := validateDeleteCommentRelationRequest(req)
	if violations != nil {
	...
	arg := db.DeleteCommentRelationParams{
		CommentID:    req.GetCommentId(),
		UserID:       authPayload.UserID,
		RelationType: relationType,
	}
	err = server.store.DeleteCommentRelation(ctx, arg)

	if err != nil {
		if db.ErrorCode(err) == db.UniqueViolation || db.ErrorCode(err) == db.ForeignKeyViolation {
			return nil, status.Errorf(codes.AlreadyExists, "comment relation not exist: %s", err)
		}
...
func validateDeleteCommentRelationRequest(req *rpcs.DeleteCommentRelationRequest) (violations []*errdetails.BadRequest_FieldViolation) {
